...
#define OBJECT_MODEL_FUNC(...) OBJECT_MODEL_FUNC_BODY(ArcitypeKinematics, __VA_ARGS__)

constexpr ObjectModelTableEntry ArcitypeKinematics::objectModelTable[] =
{
...
constexpr uint8_t ArcitypeKinematics::objectModelTableDescriptor[] = { 1, 1 };

DEFINE_GET_OBJECT_MODEL_TABLE_WITH_PARENT(ArcitypeKinematics, ZLeadscrewKinematics)

#endif

ArcitypeKinematics::ArcitypeKinematics() noexcept
	: ZLeadscrewKinematics(KinematicsType::arcitype, SegmentationType(true, false, false)),
...
const char *ArcitypeKinematics::GetName(bool forStatusReport) const noexcept
{
	return "Arcitype";
}
...
bool ArcitypeKinematics::CalculateThetaAndPsi(const float machinePos[], bool isCoordinated, float& theta, float& psi, bool& armMode) const noexcept
{
	...
	// ARCITYPE position is undefined if abs(ARCITYPE_C2) >= 1. In reality abs(ARCITYPE_C2) >0.95 can be problematic.
	...
	const float ARCITYPE_K1 = proximalArmLength + distalArmLength * cosPsi;
	const float ARCITYPE_K2 = distalArmLength * sinPsi;
	...
	for (;;)
	{
		if (armMode != switchedMode)
		{
			...
			{
				theta = atan2f(ARCITYPE_K1 * y - ARCITYPE_K2 * x, ARCITYPE_K1 * x + ARCITYPE_K2 * y) * RadiansToDegrees;
...
bool ArcitypeKinematics::CartesianToMotorSteps(const float machinePos[], const float stepsPerMm[], size_t numVisibleAxes, size_t numTotalAxes, int32_t motorPos[], bool isCoordinated) const noexcept
{
	float theta, psi;
...
// For Arcitype, the X and Y components of stepsPerMm are actually steps per degree angle.
void ArcitypeKinematics::MotorStepsToCartesian(const int32_t motorPos[], const float stepsPerMm[], size_t numVisibleAxes, size_t numTotalAxes, float machinePos[]) const noexcept
{
...
bool ArcitypeKinematics::Configure(unsigned int mCode, GCodeBuffer& gb, const StringRef& reply, bool& error) THROWS(GCodeException) /*override*/
{
	if (mCode == 669)
	{
...
bool ArcitypeKinematics::IsReachable(float axesCoords[MaxAxes], AxesBitmap axes) const noexcept
{
...
LimitPositionResult ArcitypeKinematics::LimitPosition(float finalCoords[], const float * null initialCoords,
...
void ArcitypeKinematics::GetAssumedInitialPosition(size_t numAxes, float positions[]) const noexcept
{
...
AxesBitmap ArcitypeKinematics::AxesAssumedHomed(AxesBitmap g92Axes) const noexcept
{
...
AxesBitmap ArcitypeKinematics::MustBeHomedAxes(AxesBitmap axesMoving, bool disallowMovesBeforeHoming) const noexcept
{
...
AxesBitmap ArcitypeKinematics::GetHomingFileName(AxesBitmap toBeHomed, AxesBitmap alreadyHomed, size_t numVisibleAxes, const StringRef& filename) const noexcept
{
...
#if HAS_MASS_STORAGE
		// Some ARCITYPE printers cannot have individual axes homed safely. So it the user doesn't provide the homing file for an axis, default to homeall.
...
bool ArcitypeKinematics::QueryTerminateHomingMove(size_t axis) const noexcept
{
...
void ArcitypeKinematics::OnHomingSwitchTriggered(size_t axis, bool highEnd, const float stepsPerMm[], DDA& dda) const noexcept
{
	switch (axis)
	{
...
bool ArcitypeKinematics::IsContinuousRotationAxis(size_t axis) const noexcept
{
...
AxesBitmap ArcitypeKinematics::GetLinearAxes() const noexcept
{
	return (crosstalk[1] == 0.0 && crosstalk[2] == 0.0) ? AxesBitmap::MakeFromBits(Z_AXIS) : AxesBitmap();
}

// Recalculate the derived parameters
void ArcitypeKinematics::Recalc() noexcept
{
